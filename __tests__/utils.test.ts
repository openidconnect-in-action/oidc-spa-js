...
describe('utils', () => {
  ...
  describe('shouldNotSetSameSiteNone', () => {
    expect.extend({
      toGetSameSiteNone(userAgent) {
        const pass = !shouldNotSetSameSiteNone(userAgent);
        if (pass) {
          return {
            message: () => `"${userAgent}" should get SameSite=None`,
            pass: true
          };
        } else {
          return {
            message: () => `"${userAgent}" should not get SameSite=None`,
            pass: false
          };
        }
      }
    ...
    it('should identify browsers that should set SameSite=None', () => {
      const userAgents = [
        ...
        'Some unrecognized UA',
        null
      ];
      for (let userAgent of userAgents) {
        (expect(userAgent) as any).toGetSameSiteNone();
      }
    });

    it('should identify browsers that should not set SameSite=None', () => {
      const userAgents = [
      ...
      for (let userAgent of userAgents) {
        (expect(userAgent) as any).not.toGetSameSiteNone();
      }
    });
  });
});
